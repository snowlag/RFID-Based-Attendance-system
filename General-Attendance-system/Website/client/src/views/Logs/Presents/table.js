...
import {  GetAllDevices , GetTimeInLogs} from "../../../actions/general";
...
useEffect( () => {
    if(props.list && props.list.data && props.list.data.length > 0){
        ...
        props.GetTimeInLogs(props.list.data[0].id , {})
        setFlag(true)
    }   
...
const deleteItem = (event , item) => {
  ...
  .post("/api/delete/item" , {item})
   .then(res => {
    reset();
    var items = {}
    props.GetTimeInLogs(SelectedDepartment , {})
   })
   .catch(err => {
...
useEffect( () => {
   ...
   setIndex(index+1)
   setArr([ ...arr , props.presentlogs.Key])
   prevItems.push.apply(prevItems, props.presentlogs.Data);
  setLoader(null)
  prevItems.map(data => {
    var obj = DateConverter(data.timein)
    data.timein = obj.time
  })
  setData(prevItems);
    }
   }, [props.presentlogs])  
...
const handleOpen = (event ,data) => {
 ...
 // Load more Button to store previous state data and query for next paginated data
  ...
  // Pass evaluated key and state contained dates
      props.GetTimeInLogs( SelectedDepartment ,lastIndexKey) 
     }
    }
...
useEffect( () => {
  if(props.presentlogs.Key){
    setNext(false)
  }else{
    setNext(true)
  }
   
 } ,[props.presentlogs.Key])
...
const HandleDepartmentChange = (event) => {
    ...
    reset();
    props.GetTimeInLogs( event.target.value ,{}) 
}


  return (
    <div>
          <Toolbar Devices = {Devices} Data = {data} SelectedDepartment = {SelectedDepartment} HandleDepartmentChange = {HandleDepartmentChange} />
        <br></br>
       {loader} 
      <MaterialTable
         title="Active Users"
      ...
      onChangePage={(page) => {
        ...
        actions={[
          {
            icon: () =>   <LightTooltip title="Delete Log"><DeleteOutline /></LightTooltip>,
            onClick: deleteItem
          },          
...
const mapStateToProps =(state) =>({
  auth: state.auth,
  errors: state.errors,
 presentlogs: state.presentlogs,
  list: state.alldevicelist
});

export default connect(mapStateToProps, { GetAllDevices , GetTimeInLogs})(ArchiveUsertable);