...
useEffect( () => {
    if(props.list && props.list.data && props.list.data.length > 0){
        ...
        props.GetpaginatedUsers(props.list.data[0].id , {})
        setFlag(true)
    }   
...
const deleteItem = (event , item) => {
  ...
  .post("/api/delete/item" , {item})
   .then(res => {
    reset();
    var items = {}
    props.GetpaginatedUsers(SelectedDepartment , {})
   })
   .catch(err => {
...
const handleOpen = (event ,data) => {
 ...
 // Load more Button to store previous state data and query for next paginated data
  ...
  // Pass evaluated key and state contained dates
      props.GetpaginatedUsers( SelectedDepartment ,lastIndexKey) 
     }
    }
...
const HandleDepartmentChange = (event) => {
    ...
    reset();
    props.GetpaginatedUsers( event.target.value ,{}) 
}


  return (
    <div>
          <Toolbar Devices = {Devices} SelectedDepartment = {SelectedDepartment} HandleDepartmentChange = {HandleDepartmentChange} />
        <br></br>
       {loader} 
      <MaterialTable
         title=""
