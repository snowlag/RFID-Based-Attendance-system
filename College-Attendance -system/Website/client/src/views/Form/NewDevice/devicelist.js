...
import {  GetSubjectList } from "../../../actions/general";
...
const [deviceData] = React.useState(null)
...
useEffect( () => {
 ...
 // Load more Button to store previous state data and query for next paginated data
  ...
  // Pass evaluated key and state contained dates
      props. GetSubjectList(lastIndexKey) 
     }
    }
  ...
  return (
    <div>
      ...
      onChangePage={(page) => {
        ...
        actions={[
          {
            icon: () =>   <LightTooltip title="Delete Subject"><DeleteOutline /></LightTooltip>,
            onClick: deleteItem
          },
...
DeviceListTable.propTypes ={
     GetSubjectList: propTypes.func.isRequired
};
...
export default connect(mapStateToProps, { GetSubjectList})(DeviceListTable);