...
import { GetAllSubjectList ,GetpaginatedLectures} from "../../actions/general";
...
const [state, setState] = React.useState({
    columns: [
     { title: "Subject", field: "subjectname" },
      { title: "Day", field: "day" },
      { title: "Start time", field: "start_formatted_time" }, 
      { title: "End time", field: "end_formatted_time" }, 
 ...
 useEffect( () => {
    ...
    setFlag(true)
    props.GetpaginatedLectures(SelectedDay , {})
...
useEffect( () => {
   ...
   setIndex(index+1)
   setArr([ ...arr , props.lectures.Key])
   prevItems.push.apply(prevItems, props.lectures.Data);
  setLoader(null)
  prevItems.map(data => {
...
const handleOpen = (event ,data) => {
 ...
 // Load more Button to store previous state data and query for next paginated data
  ...
  props.GetpaginatedLectures(SelectedDay ,lastIndexKey )
     }
    }
...
const HandleDayChange = (event) => {
  ...
  reset();
  props.GetpaginatedLectures(event.target.value ,{})
}


  return (
    <div>
       <AddLectureForm  Devices = {Devices}    AddLecture = {AddLecture} />
       <br></br>
          <Toolbar Days = {days} SelectedDay = {SelectedDay} HandleDayChange = {HandleDayChange} />
        <br></br>
       {loader} 
      <MaterialTable
         title=""
...
export default connect(mapStateToProps, { GetAllSubjectList,GetpaginatedLectures })(ArchiveUsertable);